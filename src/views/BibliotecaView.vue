...
const validateName = async () => {
  ...
  if (response.status) {
    ...
    let res = files.find(
      (objeto) => objeto.Name === file.value.name && objeto.Path === folder.value
    )
    return res
  }
  return true
...
const upload = async () => {
  try {
    ...
    if (folder.value === 'BLS' || folder.value === 'Poliza-Responsabilidad-Civil') {
      if (!validarFecha()) {
        notify({
          title: '"La fecha no es válida o no es posterior a la fecha actual.',
          type: 'error'
        })
    ...
    if (
      ...
      return (isValidating.value = false)
    let userName = user.value.username.split('@')[0]
    let data = {
      path: `${year.value}/${userName}`
    }
  ...
  <div>
          ...
          <div class="col-6">
            <div class="d-block py-1">
              <div class="d-flex justify-content-end">
                <div class="img-div">
                  <img
                    class="img-header-icon ml-3 mx-1 pt-2 pt-md-0"
          ...
          <div class="col-sm-12 mt-3">
                ...
                <ul class="nav nav-tabs" id="ResultsTab" role="tablist">
                  <li class="nav-item tab-hmv2" role="presentation">
                    <button
                      ...
                      aria-selected="true"
                    >
                      Mis Documentos <span class="bg-number">{{ total_documents }}</span>
                    </button>
                  </li>
                  <li class="nav-item tab-hmv2" role="presentation">
                    <button
            ...
            <div class="tab-content" id="ResultsTabContent">
                      ...
                      <template v-else>
                          ...
                          <div class="accordion accordion-flush" id="accordionFlush">
                            <div class="row">
                              <div class="col-12 col-md-4 p-2">
                                ...
                                <select
                                  class="form-control form-select p-3 my-2"
                                  v-model="selectedYear"
                                  @change="refresh"
                                >
                                  <option :value="null">Seleccionar año</option>
                                  <option
                                    v-for="(year, yearKey) in fullYears"
                                    :key="yearKey"
                                    :value="year"
                                  >
                                    {{ year }}
                            ...
                            <template v-for="(file, documentKey) in folders" :key="documentKey">
                              <div class="accordion-item py-2 px-1">
                                <h2 class="accordion-header" :id="'flush-heading' + documentKey">
                                  <button
                                    ...
                                    :data-bs-target="'#flush-collapseOne' + documentKey"
                                    aria-expanded="false"
                                    :aria-controls="'#flush-collapseOne' + documentKey"
                                  >
                                    {{ selectedYear }}/{{ file.label }}
                                    <span class="bg-number mx-2"
                                      ><b>{{ file.data.length }}</b></span
                                    >
                                  </button>
                                </h2>
                                <div
                                  :id="'flush-collapseOne' + documentKey"
                                  class="accordion-collapse collapse"
                                  :aria-labelledby="'flush-heading' + documentKey"
                                      ...
                                      <template v-if="file.data.length > 0">
                                        <div class="row">
                                          ...
                                          >
                                                ...
                                                <th>
                                                  <div
                                                    ...
                                                    :class="{
                                                      ...
                                                      'col-6':
                                                        file.name !== 'BLS' &&
                                                        file.name !== 'Poliza-Responsabilidad-Civil'
                                                    }"
                                                  >
                                                    Descripción
                                                  </div>
                                            ...
                                            <tbody
                                              v-for="(archive, archiveKey) in file.data"
                                              :key="archiveKey"
                                            >
                                              <tr>
                                                <td>
                                                  <h6 class="text-left">{{ archive.Name }}</h6>
                                                </td>
                                                <td>
                                                  <h6
                                                    class="text-left"
                        ...
                        <template v-else>
                          <div class="my-3 py-3 text-center">
                            <h4 class="center text-search">No tienes documentos</h4>
                          </div>
                        </template>
                      </template>
                    </div>
              ...
              <div
                ...
                role="tabpanel"
                :class="{
                  active: myLibraryStore.activeTab === 1,
                  show: myLibraryStore.activeTab === 1
                }"
                        ...
                        <div class="row">
                          <div class="col-12 col-md-4 p-2">
                            ...
                            >
                              ...
                              <option v-for="(year, yearKey) in years" :key="yearKey" :value="year">
                                {{ year }}
                              </option>
                            </select>
                            <p v-if="year === null && dirty" class="mx-1 error-text">
                              El año es requerido
                            </p>
                          </div>
                          <div class="col-12 col-md-4 p-2">
                            ...
                            <select
                              class="form-control p-3 my-2 form-select"
                              v-model="folder"
                              :disabled="isValidating"
                            >
                            ...
                            <p v-if="folder === null && dirty" class="mx-1 error-text">
                              La carpeta es requerida
                            </p>
                          </div>
                          <div
                          ...
                          >
                            ...
                            <input
                              ...
                              :disabled="isValidating"
                            />
                            <p v-if="date === null && dirty" class="mx-1 error-text">
                        ...
                        <div class="my-2 py-2 text-center">
                          <template v-if="file">
                            ...
                            <div class="files">
                              ...
                              <div class="d-flex justify-content-start">
                                <p
                                  v-if="(file.name === null || file.name === '') && dirty"
                                  class="mx-5 my-2 error-text"
                                >
                                  El nombre es requerido
                                </p>
                                <p v-if="exist && dirty" class="mx-5 my-2 error-text">
                                  El nombre ya existe
                                </p>
                              </div>
                              ...
                              <div class="d-flex">
                                ...
                                <textarea
                                  ...
                                  :disabled="isValidating"
                                  minlength="50"
                                  placeholder="Descripción del archivo de mínimo 50 y máximo 100 caracteres"
                                ></textarea>
                              </div>
                              <div class="d-flex justify-content-start">
                                <p
                                  v-if="(description === null || description === '') && dirty"
                                  class="mx-5 my-2 error-text"
                                >
                                ...
                                <p
                                  v-if="
                                    (description?.length > 100 || description?.length < 50) && dirty
                                  "
                                  class="mx-5 my-2 error-text"
                                >
                                  La descripción debe ser de mínimo 50 y máximo 100 caracteres
                                </p>
                              </div>

                              <div class="d-flex justify-content-center">
                                <button
                                  class="py-2 px-4 mx-2 my-2 upload-button"
                                  :disabled="isValidating"
                                  @click="upload"
                                >
                          ...
                          <template v-else>
                            ...
                            <div class="d-flex">
                              <button
                                class="py-2 px-4 mx-2 upload-button mx-auto"
                                @click="openSelector"
                              >
    ...
    <SmallBox :showing="showDeleteModal" @close="closeModal">
      <div class="p-4">
        ...
        <template v-else>
          <h6>
            <b>¿Estás seguro de eliminar el archivo {{ selectedArchive.Name }}?</b>
          </h6>
