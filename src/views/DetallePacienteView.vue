...
const cardiology_results = computed(() => patientDetailStore.cardiology_results)
const pathology_results = computed(() => patientDetailStore.pathology_results)
...
const isLoadingCardiology = ref(false)
const isLoadingPathology = ref(false)
...
const getPatientEV = async (nhc) => {
  try {
    ...
    if (evPacienteResponse.status) {
      ...
      // get patient forms
      patientDetailStore.formsPaciente = await formularioPaciente(
        evPaciente.value[0].NHCL,
        evPaciente.value[0].ADM
      )
...
const getCardiologyResults = async (nhc) => {
  // get patient lab results
  try {
    if (patientDetailStore.cardiology_results.length < 1) {
      isLoadingCardiology.value = true
    }
    const response = await resultadosCardiologiaPaciente(nhc)
    if (response.status) {
      patientDetailStore.cardiology_results = response.data
    } else {
      patientDetailStore.cardiology_results = []
      notify({
        title: '¡Atención!',
        text: 'No existen resultados de cardiología disponibles para este paciente',
        type: 'info'
      })
    }
    isLoadingCardiology.value = false
  } catch (e) {
    isLoadingCardiology.value = false
    notify({
      title: '¡Error!',
      text: 'No se pudo cargar los resultados de cardiología para este paciente',
      type: 'error'
    })
  }
}
const getPathologyResults = async (nhc) => {
  ...
  try {
    ...
    if (response.status) {
      ...
      patientDetailStore.pathology_results = []
      notify({
        title: '¡Atención!',
...
const downloadCardiologyFile = (result) => {
  if (result.urlPdf) {
    ...
    urlDocumentoCardiologia(id).then(async (response) => {
      if (response.status) {
        const link = document.createElement('a')
        link.setAttribute('href', response.url)
        // link.setAttribute('target', '_blank')
        link.setAttribute('download', `${result.estudio}.pdf`)
        ...
        notify({
          title: 'Listo',
          text: 'Resultado de Cardiología descargado',
          type: 'success'
        })
        event('file_download', {
          value: 'cardiología'
...
const goToCardiologyResult = async (result) => {
  ...
  await router.push({
    ...
    params: { url: id, nhc: encryptedNHC.value },
    query: { prev: 'detalle-paciente' }
  })
}
const goToCardiologyResultCtrl = async (result) => {
  const id = new URL(result.urlPdf).searchParams.get('id');
  event('see_cardiology_result', {
    nhc: nhc.value,
    uuid: id
          ...
          <div class="col-6">
            <div class="d-block py-1">
              <div class="d-flex justify-content-end">
                <div class="img-div">
                  <img
                    class="img-header-icon mx-1 ml-3"
                    src="@/assets/mis-pacientes.png"
                    alt=" icon"
                  />
                </div>
                <br />
                <h4
                  ...
                  style="color: #05305d; text-align: left; font-weight: 600"
                >
        ...
        <div class="row mt-3">
          <div class="d-block py-1">
            <div class="d-flex justify-content-start">
              ...
              <br />
              <h4
                class="d-flex text-headerv2 text-name-paciente mt-3"
            ...
            <div class="row my-2 dates-pacientes">
              <div class="col-12 col-md-5">
                ...
                <p class="descrip-paciente">
                  <b>Diagnóstico: </b
                  >{{
                    statusPaciente?.DG_PRINCIPAL ? statusPaciente?.DG_PRINCIPAL : 'No registrado'
                  }}
                </p>
                <p class="descrip-paciente">
                  <b>Plan/Convenio: </b
                  >{{ statusPaciente?.TIPO_DCTO ? statusPaciente?.TIPO_DCTO : 'No registrado' }}
                </p>
              </div>
              <div class="col-12 col-md-4">
                <p class="descrip-paciente">
                  <b>Médico: </b
                  >{{
                    statusPaciente?.NOMBRE_MEDICO ? statusPaciente?.NOMBRE_MEDICO : 'No registrado'
                  }}
                </p>
                <p class="descrip-paciente">
                  <b>Especialidad: </b
                  >{{
                    statusPaciente?.ESPECIALIDAD ? statusPaciente?.ESPECIALIDAD : 'No registrado'
                  }}
                </p>
                <p class="descrip-paciente">
                  <b>Atención: </b
              ...
              <div class="col-12 col-md-3">
                <p class="descrip-paciente">
                  <b>Edad: </b
                  >{{ statusPaciente?.EDAD ? statusPaciente?.EDAD + ' Años' : 'No registrado' }}
                </p>
                <p class="descrip-paciente">
                  <b>CI: </b>{{ statusPaciente?.CEDULA ? statusPaciente?.CEDULA : 'No registrado' }}
                </p>
                <p class="descrip-paciente">
                  <b>Ubicación: </b
                  >{{
                    statusPaciente?.NRO_HABITACION
                      ? statusPaciente?.NRO_HABITACION
                      : 'No registrado'
                  }}
                </p>
          ...
          <div class="col-sm-12 mt-2">
            ...
            <ul class="nav nav-tabs tabs-details" id="PatientTab" role="tablist">
              ...
              <li class="nav-item tab-hm" role="presentation">
                <button
                  ...
                  id="cardiology-tab"
                  data-toggle="tab"
                  :class="{ active: patientDetailStore.activeTabPaciente === 4 }"
                  @click="patientDetailStore.activeTabPaciente = 4"
                  data-target="#cardiology"
                  type="button"
                  role="tab"
            ...
            <div class="tab-content" id="PatientTabContent">
              <div
                ...
                role="tabpanel"
                :class="{
                  active: patientDetailStore.activeTabPaciente === 0,
                  show: patientDetailStore.activeTabPaciente === 0
                }"
                    ...
                    <div class="col-12 py-4 pl-0">
                      ...
                      <template v-else>
                        <template v-if="svPaciente.length > 0">
                          <div class="row">
                            <div
                              class="col-12 col-md-4 mt-3 mb-2 mb-md-5"
                            ...
                            >
                              <div class="row bg-box-results">
                                ...
                                <div class="col-9 col-md-9">
                                  ...
                                  <p class="detail-paciente" v-if="item.VALOR">
                                    {{ item.VALOR }} {{ getUnit(item) }}
                                  </p>
                                ...
                                <div class="col-12">
                                  <template v-if="item.SIGNO === 'TEMPERATURA'">
                                    <div
                                      @click.exact="goToGraphic('TEMP', item)"
                                      @click.ctrl="goToGraphicCtrl('TEMP', item)"
                                      class="cursor-pointer text-center"
                                    >
                                      <p class="title-paciente" style="text-decoration: underline">
                                  ...
                                  <template
                                    v-if="
                                      item.SIGNO === 'PRESION ARTERIAL SISTOLICA' ||
                                      item.SIGNO === 'PRESION ARTERIAL DIASTOLICA'
                                    "
                                  >
                                    <div
                                      @click.exact="goToGraphic('PSD', item)"
                                      @click.ctrl="goToGraphicCtrl('PSD', item)"
                                      class="cursor-pointer text-center"
                                    >
                                      <p
                                        class="title-paciente"
                                  ...
                                  <template v-if="item.SIGNO === 'FRECUENCIA CARDIACA'">
                                    <div
                                      @click.exact="goToGraphic('FC', item)"
                                  ...
                                  <template v-if="item.SIGNO === 'GLICEMIA CAPILAR'">
                                    <div
                                      @click.exact="goToGraphic('GLUC', item)"
              ...
              <div
                ...
                role="tabpanel"
                :class="{
                  active: patientDetailStore.activeTabPaciente === 1,
                  show: patientDetailStore.activeTabPaciente === 1
                }"
                      ...
                      <template v-else>
                        <template v-if="formsPaciente.length > 0">
                          <template v-for="(form, i) in formsPaciente" v-bind:key="i">
                            <div class="my-1 py-4 px-2 text-left border-result">
                              <div class="row">
                                <div class="col-12 col-md-12">
                                  ...
                                  >
                                    <div>
                                      ...
                                      <p class="title-results title-colapse">
                                        <b>Fecha: </b>{{ form.fecha_FOR }}
                                      </p>
                                      <p class="title-results title-colapse">
                                        <b>Médico: </b>{{ form.med_FOR }}
                                      </p>
                                      <p class="title-results title-colapse">
                                        <b>Número Historia Clínica: </b>{{ statusPaciente?.HC }}
                                      </p>
                                      <p class="title-results title-colapse">
                                        <b>Número de Atención: </b>{{ evPaciente[0]?.ADM }}
                                      </p>
                                    </div>
                                    <div class="d-flex flex-column align-items-end px-3">
                                      <div>
                                        <font-awesome-icon
                                          :icon="[
                                            'fas',
                                            form.visible ? 'chevron-up' : 'chevron-down'
                                          ]"
                                        />
                                      </div>
                                    </div>
              ...
              <div
                ...
                role="tabpanel"
                :class="{
                  active: patientDetailStore.activeTabPaciente === 2,
                  show: patientDetailStore.activeTabPaciente === 2
                }"
                          ...
                          >
                            <div class="col-9">
                              <p class="title-results">
                                <b>{{ labResult?.ORIGEN }}</b>
                                <span
                                  class="p-2 mx-2 pill"
                                  v-if="isWithin24Hours(labResult?.fecha_)"
                                  :class="{
                                    gray:
                                      labResult?.IsOrderValidated === 'false' ||
                                      !labResult?.IsOrderValidated,
                                    orange:
                                      labResult?.IsOrderValidated ||
                                      labResult?.IsOrderValidated === 'true'
                                  }"
                                >
                              ...
                              <p class="text-results"><b>Médico:</b> {{ labResult?.MEDICO }}</p>
                              <div
                                class="flex cursor-pointer"
                                @click="labResult.isOpen = !labResult.isOpen"
                              >
                              ...
                              <template v-if="labResult.isOpen">
                                ...
                                <div class="row px-3 py-1">
                                  <ul
                                    ...
                                    v-for="(exam, key) in labResult.examenes"
                                    v-bind:key="key"
                                  >
                            ...
                            <div class="col-3 d-flex flex-column flex-md-row justify-content-end">
                              ...
                              <div
                                @click="downloadLabFile(labResult)"
                                class="cursor-pointer"
              ...
              <div
                ...
                role="tabpanel"
                :class="{
                  active: patientDetailStore.activeTabPaciente === 3,
                  show: patientDetailStore.activeTabPaciente === 3
                }"
                  ...
                  <div class="row justify-content-center">
                    <div class="col-12 py-4">
                      ...
                      <template v-else>
                        <template v-if="seeImages">
                          ...
                          <template v-if="image_results.length > 0">
                            ...
                            >
                              <div class="col-9">
                                <p class="title-results">
                                  <b>{{ imageResult?.ESTUDIO }}</b>
                                  <span
                                    class="p-2 mx-2 pill"
                                    v-if="isWithin24Hours(imageResult?.fecha_)"
                                    >Nuevo</span
                                  >
                                </p>
                                <p class="text-results">{{ imageResult?.FECHA }}</p>
                                <p class="text-results"><b>Médico:</b> {{ imageResult?.MEDICO }}</p>
                        ...
                        <template v-else>
                          <div
                            @click.exact="goToZeroCtrl()"
                            @click.ctrl="goToZeroCtrl()"
                            class="my-1 p-4 row text-left border-result hover-list-element cursor-pointer"
                          >
                            <p class="title-results m-4"><b>Ver Exámenes (Zero FootPrint GE)</b></p>
                          </div>
                          <div
                            @click="patientDetailStore.seeImages = true"
                            class="my-1 p-4 row cursor-pointer text-left border-result hover-list-element"
                          >
                            <p class="title-results m-4"><b>Ver Informes</b></p>
                          </div>
                        </template>
                      </template>
                    </div>
              ...
              <div
                ...
                role="tabpanel"
                :class="{
                  active: patientDetailStore.activeTabPaciente === 4,
                  show: patientDetailStore.activeTabPaciente === 4
                }"
              ...
              >
                <div class="container">
                  <div class="row justify-content-center">
                    <div class="col-12 py-4">
                      <template v-if="isLoadingCardiology">
                        <div class="d-flex justify-content-center">
                          <img class="img-fluid" src="@/assets/loading.gif" alt="Loading Hm" />
                        </div>
                      </template>
                      <template v-else>
                        <template v-if="cardiology_results.length > 0">
                          <div
                            class="my-1 py-2 row text-left border-result hover-list-element"
                            v-for="(cardiologyResult, cardiologyResultKey) in cardiology_results"
                            :key="cardiologyResultKey"
                          >
                            <div class="col-9">
                              <p class="title-results">
                                <b>{{ cardiologyResult?.estudio }}</b>
                              </p>
                              <p class="text-results">{{ cardiologyResult?.fechaHora }}</p>
                              <p class="text-results"><b>Médico:</b> {{ cardiologyResult?.nombreMedico }}</p>

                              <p style="color: #ff8201" class="text-results">Cardiología</p>
                            </div>
                            <div class="col-3 d-flex flex-column flex-md-row justify-content-end">
                              <div
                                class="cursor-pointer"
                                title="Ver resultado"
                                style="display: inline-block"
                                @click.exact="goToCardiologyResult(cardiologyResult)"
                                @click.ctrl="goToCardiologyResultCtrl(cardiologyResult)"
                              >
                                <div class="py-3 p-md-4">
                                  <font-awesome-icon
                                    :icon="['fas', 'eye']"
              ...
              <div
                ...
                role="tabpanel"
                :class="{
                  active: patientDetailStore.activeTabPaciente === 5,
                  show: patientDetailStore.activeTabPaciente === 5
                }"
                    ...
                    <div class="col-12 py-4">
                      ...
                      <template v-else>
                        ...
                        <template v-else>
                          <div class="my-3 py-3 text-center">
                            <h4 class="center text-search">
                              No existen resultados de patología disponibles para este paciente.
                            </h4>
                          </div>
                        </template>
                      </template>
