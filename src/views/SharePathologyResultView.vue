...
onMounted(async () => {
  ...
  screenview('Compartir Resultado de Patología')
  getUrl(url.value)
})
    ...
    statusPacienteEmergencia(nhc)
      .then((response) => {
        if (response.status) {
          ...
          title.value = `Resultado de Patología ${statusPaciente.value.NOMBRE_PACIENTE} - Metrovirtual - Hospital Metropolitano`
        }
      })
      .catch((e) => {
        console.log('error', e)
...
const getUrl = async (url) => {
  ...
  try {
    ...
    const response = await urlDocumentoPatologiaShare(url)
